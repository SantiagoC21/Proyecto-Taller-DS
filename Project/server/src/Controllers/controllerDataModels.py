...
def controllerData():
    ...
    try:
        ...
        return [{ 'message': f'Error en consulta de base de datos: {str(e)}' }]
    ...
    if isinstance(response, list) and response and 'message' in response[0]:
        ...
        for nombre_archivo in archivos_mdl:
            ...
            for i in response_format:
                try:
                    ...
                    nivel[modelo][submodelo][variable] = {
                        'data': stock_data,
                        'titulo': i['tituloVariable'],
                        'tipo': i['tipoVariable'],
                        'unidad': i['unidadVariable']
                    }
                except Exception as e:
                    return [{'message': f"Error graficando variable {i['nameNivel']} desde archivo {nombre_archivo}: {str(e)}"}]
